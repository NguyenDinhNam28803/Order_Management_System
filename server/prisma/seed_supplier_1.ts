...
async function main() {
    ...
    const supplierOrg = await prisma.organization.upsert({
        ...
        create: {
            code: 'NCC_DIEN_TU',
            name: 'Nhà cung cấp thiết bị điện tử 1',
            email: 'hunglctb00380@fpt.edu.vn',
            companyType: CompanyType.SUPPLIER,
    ...
    const supplierUser = await prisma.user.upsert({
        where: { email: 'hunglctb00380@fpt.edu.vn' },
        update: {
            fullName: 'Nhà cung cấp thiết bị điện tử 1',
    ...
    console.log(`- Email: hunglctb00380@fpt.edu.vn`);
