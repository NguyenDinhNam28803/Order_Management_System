...
async function seedBudgetForTesting() {
  ...
  console.log(`Found Cost Center: ${costCenter.name} (ID: ${costCenter.id}) for Org: ${orgId}`);
  ...
  const periods = [
    { type: 'ANNUAL', num: 1, start: `${fiscalYear}-01-01`, end: `${fiscalYear}-12-31` },
    { type: 'QUARTERLY', num: 1, start: `${fiscalYear}-01-01`, end: `${fiscalYear}-03-31` },
    { type: 'QUARTERLY', num: 2, start: `${fiscalYear}-04-01`, end: `${fiscalYear}-06-30` },
    { type: 'QUARTERLY', num: 3, start: `${fiscalYear}-07-01`, end: `${fiscalYear}-09-30` },
    { type: 'QUARTERLY', num: 4, start: `${fiscalYear}-10-01`, end: `${fiscalYear}-12-31` },
  ];
  ...
  for (const p of periods) {
    ...
    console.log(`Successfully seeded ${p.type} ${p.num} with ${amount.toLocaleString()} VND`);
  }
