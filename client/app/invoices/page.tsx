    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-xl border border-[#E2E8F0] bg-[#F1F5F9] p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-base font-bold text-[#0F172A]"><Sparkles size={16} className="text-[#2563EB]" />Nhập hoá đơn bằng AI</h2>
          <button onClick={onClose}><X size={16} className="text-[#64748B] hover:text-[#0F172A]" /></button>
        </div>
        <p className="mb-4 text-xs text-[#64748B]">Dán nội dung hoá đơn từ email, Zalo, hoặc ghi chú qua điện thoại. AI sẽ tự trích xuất số liệu và tạo hoá đơn DRAFT để bạn kiểm tra lại.</p>
        ...
        {result ? (
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-sm text-black space-y-1">
            ...
            <p className="text-xs text-[#64748B]">Độ tin cậy AI: {((result.aiConfidence ?? 0) * 100).toFixed(0)}% · {result.message}</p>
            <button onClick={onClose} className="mt-2 text-xs text-[#3B82F6] hover:underline">Đóng</button>
          </div>
        ) : (
          <div className="space-y-3">
            <div>
              <label className="mb-1 block text-xs text-[#64748B]">Nội dung hoá đơn *</label>
              <textarea rows={6} value={rawText} onChange={e => setRawText(e.target.value)} placeholder={"Ví dụ:\nHoá đơn số: HD-2026-055\nNgày: 24/04/2026\nPO: PO-2026-012\nThành tiền: 15,000,000 VND\nThuế 10%: 1,500,000 VND\nTổng cộng: 16,500,000 VND"} className="w-full rounded-lg border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-2 text-xs text-[#000000] placeholder:text-[#94A3B8] focus:outline-none focus:border-purple-500/50 resize-none" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-xs text-[#64748B]">Supplier ID *</label>
                <input value={supplierId} onChange={e => setSupplierId(e.target.value)} placeholder="UUID nhà cung cấp" className="w-full rounded-lg border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-2 text-xs text-[#000000] placeholder:text-[#94A3B8] focus:outline-none focus:border-purple-500/50" />
              </div>
              <div>
                <label className="mb-1 block text-xs text-[#64748B]">Org ID *</label>
                <input value={orgId} onChange={e => setOrgId(e.target.value)} placeholder="UUID tổ chức" className="w-full rounded-lg border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-2 text-xs text-[#000000] placeholder:text-[#94A3B8] focus:outline-none focus:border-purple-500/50" />
              </div>
            </div>
            <div className="flex justify-end gap-3 pt-1">
              <button onClick={onClose} className="px-4 py-2 text-xs text-[#64748B] hover:text-[#0F172A]">Huỷ</button>
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-[#0F172A] tracking-tight">Quản lý Hóa đơn</h1>
          <p className="text-xs text-[#64748B] mt-0.5">Theo dõi và xử lý tất cả hóa đơn nhà cung cấp</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowAiModal(true)}
            className="flex items-center gap-2 rounded-xl border border-purple-500/30 bg-purple-600/10 px-3 py-2 text-xs font-semibold text-purple-700 hover:bg-purple-600/20 transition-colors"
          >
            <Sparkles size={14} />
            Nhập từ AI
          </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors ${
                filterStatus === s
                  ...
                  : "bg-[#F1F5F9] text-[#0F172A] hover:text-[#0F172A] hover:bg-[#F1F5F9]/80"
              }`}
            >
                  ...
                  <tr key={inv.id}>
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-[#0F172A]">{inv.invoiceNumber ?? "—"}</span>
                      </div>
                    </td>
                    <td className="font-mono text-[#0F172A]">{inv.po?.poNumber ?? "—"}</td>
                    <td className="text-[#0F172A]">{inv.supplier?.name ?? inv.supplierName ?? "—"}</td>
                    <td className="text-right font-bold text-[#0F172A]">
                      {inv.totalAmount != null
                        ? Number(inv.totalAmount).toLocaleString("vi-VN")
                        : "—"}
                    </td>
                    <td className="text-[#0F172A] text-xs">
                      {inv.invoiceDate
                    ...
                    <td>
                      <div className="flex items-center justify-center gap-1">
                        <Link
                          ...
                          className="p-1.5 rounded-lg hover:bg-[#2563EB]/10 text-[#64748B] hover:text-[#3B82F6] transition-colors"
                          title="Xem chi tiết"
                        >
                          <Eye size={14} />
                        </Link>
                        {inv.status === "DRAFT" && (
                          <>
                            <Link
                              ...
                              className="p-1.5 rounded-lg hover:bg-amber-500/10 text-[#64748B] hover:text-amber-600 transition-colors"
                              title="Chỉnh sửa"
                            >
                              <Pencil size={14} />
                            </Link>
                            <button
                              ...
                              className="p-1.5 rounded-lg hover:bg-rose-500/10 text-[#64748B] hover:text-rose-600 transition-colors disabled:opacity-40"
                              title="Xóa"
                            >
                              {deletingId === inv.id
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-[#E2E8F0] flex items-center justify-between">
            <p className="text-xs text-[#64748B]">
              Hiển thị <span className="text-[#0F172A] font-bold">{filtered.length}</span>
              {" "}/ <span className="text-[#0F172A] font-bold">{(invoices ?? []).length}</span> hóa đơn
            </p>
          </div>
        )}
      </div>
