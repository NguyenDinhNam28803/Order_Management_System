        ...
        <main className="animate-in fade-in duration-500 min-h-screen bg-[#FFFFFF] text-slate-900">
            ...
            <header className="flex justify-between items-center bg-[#F1F5F9] p-6 rounded-2xl shadow-sm border border-[rgba(148,163,184,0.1)]">
                ...
                <Link 
                    ...
                    className="flex items-center gap-2 bg-[#2563EB] text-slate-900 px-5 py-2.5 rounded-xl font-bold hover:bg-[#1D4ED8] transition-all shadow-lg active:scale-95"
                >
                    <Plus size={20} />
            ...
            <div className="flex flex-wrap items-center gap-4 bg-[#F1F5F9] p-4 rounded-xl shadow-sm border border-[rgba(148,163,184,0.1)] justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${statusFilter === 'ALL' ? 'bg-[#2563EB] text-slate-900' : 'bg-[#FFFFFF] text-slate-900 hover:bg-[#0F172A]'}`}
                    >
                        Tất cả
                    </button>
                    {Object.values(QuoteRequestStatus).map(status => {
                        ...
                        return (
                            <button 
                                ...
                                className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${statusFilter === status ? 'bg-[#2563EB] text-slate-900' : 'bg-[#FFFFFF] text-slate-900 hover:bg-[#0F172A]'}`}
                            >
                                {label}
                            </button>
                        );
                ...
                <div className="lg:col-span-2 space-y-4">
                    {filteredQRs.length === 0 ? (
                        <div className="bg-[#F1F5F9] p-12 rounded-2xl border border-dashed border-[rgba(148,163,184,0.2)] text-center space-y-3">
                            ...
                            <h3 className="text-slate-900 font-bold italic">Không tìm thấy báo giá nào</h3>
                        </div>
                    ) : (
                        filteredQRs.map(qr => {
                                ...
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h3 className="font-black text-slate-900 group-hover:text-[#2563EB] transition-colors">{qr.title}</h3>
                                                <div className="flex flex-wrap items-center gap-2 mt-1">
                                                    <span className="text-[10px] font-black tracking-widest text-slate-900 uppercase">{qr.qrNumber}</span>
                                                    <span className="w-1 h-1 rounded-full bg-[#000000]"></span>
                                                    <span className="text-[11px] font-bold text-slate-900">Tạo: {new Date(qr.createdAt).toLocaleDateString('vi-VN')}</span>
                                                    {qr.requiredDate && (
                                                        <>
                                                            <span className="w-1 h-1 rounded-full bg-[#000000]"></span>
                    ...
                    {selectedQR ? (
                        <div className="bg-[#F1F5F9] p-6 rounded-2xl shadow-xl border border-[rgba(148,163,184,0.1)] sticky top-6 space-y-6">
                            <div className="flex justify-between items-start">
                                ...
                                <button className="p-2 text-slate-900 hover:bg-[#0F172A] rounded-lg">
                                ...
                                {selectedQR.status === QuoteRequestStatus.COMPLETED ? (
                                    <>
                                        ...
                                        <button 
                                            ...
                                            className="w-full flex items-center justify-center gap-3 bg-emerald-500 hover:bg-emerald-600 text-slate-900 py-3.5 rounded-xl font-black shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"
                                        >
                                            <ArrowRight size={20} />
                                    ...
                                    <div className="bg-[#FFFFFF] p-4 rounded-xl text-center space-y-2">
                                        <Clock className="mx-auto text-slate-900" size={24} />
