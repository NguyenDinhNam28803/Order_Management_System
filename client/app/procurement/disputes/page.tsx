    ...
    const getStatusBadge = (status: DisputeStatus) => {
        switch (status) {
            ...
            case DisputeStatus.CLOSED:
                return <span className="px-2 py-1 bg-[#F1F5F9] text-[#0F172A] border border-[#E2E8F0] rounded-full text-xs font-bold flex items-center gap-1 w-fit"><XCircle size={12}/> Đã đóng</span>;
            default:
    ...
    return (
        <main className="animate-in fade-in duration-500 p-6 min-h-screen bg-[#FFFFFF] text-[#0F172A]">
            <div className="flex justify-between items-center mb-10">
                <div>
                    <h1 className="text-2xl font-black tracking-tight text-[#0F172A]">Khiếu nại & Tranh chấp</h1>
                    <p className="text-[#64748B] text-sm">Xử lý các vấn đề phát sinh về chất lượng hàng hóa, thanh toán hoặc dịch vụ</p>
                </div>
                <button className="flex items-center gap-2 bg-rose-500 text-white px-4 py-2 rounded-xl hover:bg-rose-600 transition-all shadow-lg shadow-rose-500/20 font-semibold uppercase tracking-wider text-[11px]">
                    <AlertTriangle size={18} /> Tạo khiếu nại mới
                </button>
            </div>

            <div className="bg-[#F1F5F9] p-4 rounded-xl border border-[#E2E8F0] shadow-2xl shadow-[#2563EB]/5 flex flex-wrap gap-4 items-center mb-8">
                <div className="flex-1 min-w-[300px] flex gap-3">
                    ...
                    <div className="relative flex-1">
                        <input 
                            ...
                            className="w-full h-14 pl-6 pr-4 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl text-[#0F172A] placeholder:text-[#94A3B8]/40 focus:outline-none focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/5 transition-all text-sm font-bold"
                            value={searchTerm}
                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        <select 
                            className="h-14 pl-12 pr-10 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl text-[#0F172A] text-sm font-bold focus:outline-none focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/5 transition-all appearance-none cursor-pointer min-w-[200px]"
                            value={statusFilter}
            ...
            <div className="grid grid-cols-1 gap-4">
                {filteredDisputes.length > 0 ? filteredDisputes.map((d) => (
                    <div key={d.id} className="bg-[#F1F5F9] p-5 rounded-xl shadow-xl shadow-[#2563EB]/5 border border-[#E2E8F0] hover:border-rose-500/20 transition-all group relative">
                        <div className="flex justify-between items-start mb-3">
                            <div className="space-y-1">
                                <div className="flex items-center gap-3">
                                    <span className="font-black text-black">#{d.disputeNumber}</span>
                                    {getStatusBadge(d.status)}
                                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase ${d.priority === 'HIGH' ? 'bg-rose-500 text-white' : 'bg-[#FFFFFF] text-[#0F172A] border border-[#E2E8F0]'}`}>
                                        {d.priority}
                                    </span>
                                </div>
                            ...
                            <div className="text-right text-xs text-[#0F172A]">
                                ...
                                <p>Tài liệu: <span className="font-bold text-[#0F172A] uppercase">{d.relatedDocumentType}</span></p>
                            </div>
                        </div>
                        
                        <div className="bg-[#FFFFFF] p-3 rounded-xl text-sm text-[#0F172A] mb-4 line-clamp-2 italic border border-[#E2E8F0]">
                            &quot;{d.reason}&quot;
                        </div>
                        ...
                        <div className="flex justify-between items-center pt-4 border-t border-[#E2E8F0]">
                            <div className="flex items-center gap-4 text-xs">
                                <div className="flex items-center gap-1.5 text-[#0F172A]">
                                    ...
                                    <span>Bởi: <b className="text-[#0F172A]">{d.reportedBy?.fullName || "N/A"}</b></span>
                                </div>
                                {d.assignedTo && (
                                    <div className="flex items-center gap-1.5 text-[#0F172A]">
                                        ...
                                        <span>Xử lý: <b className="text-[#0F172A]">{d.assignedTo?.fullName}</b></span>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="bg-[#F1F5F9] p-12 rounded-xl border border-dashed border-[#E2E8F0] text-center space-y-3">
                        <div className="w-16 h-16 bg-[#FFFFFF] rounded-full flex items-center justify-center mx-auto text-[#0F172A] border border-[#E2E8F0]">
                            <MessageSquare size={32} />
                        </div>
                        <div className="text-[#0F172A] italic">
