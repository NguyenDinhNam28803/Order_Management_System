...
export default function DisputesPage() {
    ...
    const getStatusBadge = (status: DisputeStatus) => {
        switch (status) {
            ...
            default:
                return <span className="px-2 py-1 bg-blue-600/10 text-blue-600 border border-blue-600/20 rounded-full text-xs font-bold w-fit">{status}</span>;
        }
    };
            ...
            <div className="grid grid-cols-1 gap-4">
                {filteredDisputes.length > 0 ? filteredDisputes.map((d) => (
                    <div key={d.id} className="bg-slate-100 p-5 rounded-xl shadow-xl shadow-blue-600/5 border border-slate-200 hover:border-rose-500/20 transition-all group relative">
                        ...
                        <div className="bg-white p-3 rounded-xl text-sm text-slate-900 mb-4 line-clamp-2 italic border border-slate-200">
                            &quot;{d.reason}&quot;
                        </div>
