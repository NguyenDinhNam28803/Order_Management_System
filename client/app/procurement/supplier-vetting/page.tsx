...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  IN_REVIEW:        { label: "Đang kiểm tra",    color: "text-[#3B82F6] bg-[#2563EB]/10 border-[#2563EB]/20",       icon: <AlertCircle size={10} /> },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-[rgba(240,246,252,0.1)] bg-[#F1F5F9] p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-xs text-slate-900">Supplier ID (UUID) *</label>
            <input
              value={supplierId}
              onChange={(e) => setSupplierId(e.target.value)}
              placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
              className="w-full rounded-lg border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-2 text-sm text-slate-900 placeholder:text-slate-900 focus:border-[#2563EB]/50 focus:outline-none"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs text-slate-900">Giá so thị trường (%)</label>
            <input
              type="number"
              step="0.1"
              value={priceVsMarket}
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-900">Ghi chú</label>
            <textarea
              rows={3}
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              className="w-full rounded-lg border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-2 text-sm text-slate-900 placeholder:text-slate-900 focus:border-[#2563EB]/50 focus:outline-none resize-none"
            />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={loading} className="flex items-center gap-2 rounded-lg bg-[#1D4ED8] px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-[#2563EB] disabled:opacity-50">
...
export default function SupplierVettingListPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] px-6 py-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900">
            <UserCheck size={24} className="text-[#3B82F6]" />Xét duyệt Nhà cung cấp
          </h1>
          <p className="mt-1 text-sm text-slate-900">Quy trình thẩm định pháp lý, chất lượng và giá cả trước khi chính thức hợp tác</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => void load()} className="rounded-lg border border-[#E2E8F0] bg-[#F1F5F9] px-3 py-2 text-sm text-slate-900 hover:text-slate-900">
            <RefreshCw size={14} />
          </button>
          <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 rounded-lg bg-[#1D4ED8] px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-[#2563EB]">
      ...
      {/* Content */}
      {loading ? (
        <div className="flex items-center justify-center py-20"><Loader2 size={28} className="animate-spin text-[#3B82F6]" /></div>
      ) : error ? (
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-6 text-center text-sm text-red-400">{error}</div>
      ) : filtered.length === 0 ? (
        <div className="rounded-xl border border-[#E2E8F0] bg-[#F1F5F9] py-16 text-center">
          <UserCheck size={36} className="mx-auto mb-3 text-slate-900" />
          <p className="text-sm text-slate-900">Chưa có yêu cầu xét duyệt nào</p>
        </div>
      ) : (
          ...
          {filtered.map((v) => {
            ...
            return (
              <Link key={v.id} href={`/procurement/supplier-vetting/${v.id}`}>
                <div className="group flex items-center gap-4 rounded-xl border border-[#E2E8F0] bg-[#F1F5F9] p-4 hover:border-[#2563EB]/30 hover:bg-[#1C2128] transition-all cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-slate-900 truncate">{v.supplier.name}</span>
                      <StatusBadge status={v.status} />
                    </div>
                    <div className="mt-0.5 flex items-center gap-3 text-xs text-slate-900">
                  ...
                  <div className="text-right text-xs text-slate-900 shrink-0">
                    <div className="font-semibold text-slate-900">{passed}/{total} checks</div>
                    {v.overallScore != null && <div className="mt-0.5">Score: {v.overallScore}</div>}
                  </div>
                  <ChevronRight size={16} className="text-slate-900 group-hover:text-[#3B82F6] shrink-0 transition-colors" />
                </div>
              </Link>
            );
          })}
