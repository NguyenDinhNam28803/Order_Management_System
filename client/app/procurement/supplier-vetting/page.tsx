...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  IN_REVIEW:        { label: "Đang kiểm tra",    color: "text-blue-600 bg-blue-600/10 border-blue-600/20",       icon: <AlertCircle size={10} /> },
  ...
  return (
    <main className="p-6 space-y-6">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2">
            <UserCheck size={24} className="text-blue-600" />Xét duyệt Nhà cung cấp
          </h1>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-100 py-16 text-center">
          ...
          {filtered.map((v) => {
            ...
            return (
              <Link key={v.id} href={`/procurement/supplier-vetting/${v.id}`}>
                <div className="group flex items-center gap-4 rounded-xl border border-slate-200 bg-slate-100 p-4 hover:border-blue-600/30 hover:bg-slate-800 transition-all cursor-pointer">
                  ...
                  <ChevronRight size={16} className="text-slate-900 group-hover:text-blue-600 shrink-0 transition-colors" />
                </div>
              </Link>
            );
          })}
