        ...
        <main className="animate-in fade-in duration-500 p-6 min-h-screen bg-[#0F1117] text-[#F8FAFC]">
            <div className="mt-8 mb-8 border-b border-[rgba(148,163,184,0.1)] pb-4 flex justify-between items-end">
                <div>
                    ...
                    <p className="text-sm text-[#94A3B8] mt-1">Chọn PO đang chờ nhập kho bên dưới để bắt đầu đếm số lượng & kiểm định chất lượng.</p>
                </div>
            </div>
            ...
            {!activePO ? (
                <div className="space-y-4">
                    ...
                    <div className="bg-[#161922] rounded-2xl border border-[rgba(148,163,184,0.1)] shadow-xl shadow-[#3B82F6]/5 p-6">
                        <label className="text-[10px] font-black uppercase text-[#64748B] tracking-widest mb-2 block">Lọc đơn mua hàng (PO)</label>
                    ...
                    <div className="bg-[#161922] rounded-2xl border border-[rgba(148,163,184,0.1)] overflow-hidden">
                        <div className="p-4 border-b border-[rgba(148,163,184,0.1)] flex items-center justify-between">
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#64748B]">Đơn hàng chờ nhập kho (SHIPPED)</span>
                            <span className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded">{filteredPos.length} đơn</span>
                        </div>

                        {filteredPos.length === 0 ? (
                            <div className="py-16 text-center text-[#64748B] font-bold text-xs uppercase tracking-widest">
                            ...
                            <table className="erp-table text-xs m-0">
                                <thead>
                                    <tr>
                                        <th className="text-[10px] font-black uppercase tracking-widest text-[#64748B]">Mã PO</th>
                                        <th className="text-[10px] font-black uppercase tracking-widest text-[#64748B]">Nhà cung cấp</th>
                                        <th className="text-center text-[10px] font-black uppercase tracking-widest text-[#64748B]">Số mặt hàng</th>
                                        <th className="text-[10px] font-black uppercase tracking-widest text-[#64748B]">Ngày tạo</th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredPos.map((po) => (
                                        <tr key={po.id} className="hover:bg-[#0F1117] cursor-pointer" onClick={() => handleSelectPO(po)}>
                                            <td className="font-black text-[#3B82F6]">{po.poNumber || po.id.split('-').pop()}</td>
                                            <td className="font-bold text-[#F8FAFC]">{po.supplier?.name || po.vendor || "—"}</td>
                                            <td className="text-center font-bold text-[#94A3B8]">{po.items?.length ?? 0}</td>
                                            <td className="text-[#94A3B8]">{po.createdAt ? new Date(po.createdAt).toLocaleDateString('vi-VN') : "—"}</td>
                                            <td>
                                                <button className="text-[10px] font-black uppercase tracking-widest text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded hover:bg-emerald-500/20 transition-colors">
                                                    Chọn
                                                </button>
                                            </td>
                                        </tr>
                ...
                <div className="space-y-8 fade-in">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 text-xs font-bold text-[#64748B] hover:text-[#3B82F6] transition-colors uppercase tracking-widest"
                    >
                    ...
                    <div className="bg-[#161922] text-[#F8FAFC] rounded-2xl p-6 shadow-xl relative overflow-hidden flex flex-col md:flex-row justify-between items-center z-10 border border-[rgba(148,163,184,0.1)]">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-[#94A3B8] font-medium">Bên giao: <span className="font-bold text-[#F8FAFC]">{activePO.supplier?.name || activePO.vendor || "—"}</span></p>
                        </div>
