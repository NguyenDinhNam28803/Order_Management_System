    ...
    return (
        <main className="p-6 md:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 min-h-screen bg-[#F8FAFC] text-slate-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[#F1F5F9] rounded-xl p-6 border border-slate-200 shadow-xl shadow-[#2563EB]/5 flex items-center justify-between">
                    <div>
            ...
            {/* List Header */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-[#F1F5F9] p-4 rounded-xl border border-slate-200 shadow-xl shadow-[#2563EB]/5">
                <div className="relative flex-1 max-w-md w-full focus-within:ring-2 ring-[#2563EB]/20 rounded-xl transition-all">
                    ...
                    <input
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-[#FFFFFF] border-none rounded-xl text-sm font-medium text-slate-900 placeholder:text-[#94A3B8] focus:ring-0"
                        value={searchTerm}
            ...
            {filteredRequests.length === 0 ? (
                 <div className="flex flex-col items-center justify-center bg-[#F1F5F9] border border-slate-200 rounded-xl p-16 text-center shadow-xl shadow-[#2563EB]/5">
                    <div className="w-20 h-20 bg-emerald-500/10 text-emerald-600 rounded-full flex items-center justify-center mb-6">
                        <CheckCircle2 size={40} />
                    </div>
                    <h3 className="text-xl font-black text-slate-900 mb-2">Hoàn tất công việc!</h3>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    {filteredRequests.map(req => {
                        ...
                        return (
                            <div key={req.id} className="group bg-[#F1F5F9] rounded-xl border border-slate-200 p-6 shadow-xl shadow-[#2563EB]/5 hover:border-amber-400/20 transition-all duration-300">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div>
                                            <h3 className="font-black text-slate-900 text-lg leading-tight">Yêu cầu cấp NS</h3>
                                            <p className="text-xs font-bold text-slate-900 mt-1 uppercase tracking-wider">{req.id || "CHƯA CÓ MÃ"}</p>
                                        </div>
                                    </div>
                                    <span className="px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-700 text-xs font-bold tracking-tight border border-amber-500/20">
                                        CHỜ DUYỆT
                                    </span>
                                </div>
                                ...
                                <div className="space-y-4 mb-6">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            ...
                                            <p className="font-bold text-slate-900 text-sm truncate">{dept?.name || "N/A"}</p>
                                        </div>
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-1.5 text-slate-900 text-xs font-bold">
                                                <LayoutDashboard size={14} /> COST CENTER
                                            </div>
                                            <p className="font-bold text-slate-900 text-sm truncate">{cc?.name || "N/A"}</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-2 gap-4 bg-[#FFFFFF] p-4 rounded-xl border border-slate-200">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-1.5 text-slate-900 text-xs font-bold">
                                    ...
                                    <div className="w-full">
                                        ...
                                        <p className="font-medium text-slate-900 text-sm bg-amber-500/10 p-3 rounded-xl border border-amber-500/20">
                                ...
                                <div className="flex items-center gap-3 pt-6 border-t border-slate-200">
                                    <button
                                        ...
                                        className="flex-1 max-w-40 ml-auto py-2.5 px-4 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-700 hover:text-white rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 group/btn border border-emerald-500/20"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="py-2.5 px-4 bg-rose-500/10 hover:bg-rose-500 text-rose-700 hover:text-white rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 group/btn border border-rose-500/20"
                                    >
            ...
            {selectedRequest && actionType && (
                <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6">
                    ...
                    <div className="relative w-full max-w-lg bg-[#F1F5F9] rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-200">
                        <div className={`p-8 pb-10 ${actionType === "APPROVE" ? "bg-emerald-500/10" : "bg-rose-500/10"}`}>
                            <div className="w-16 h-16 rounded-xl bg-[#F1F5F9] shadow-sm flex items-center justify-center mb-6 border border-slate-200">
                                {actionType === "APPROVE" ? (
                            ...
                            <p className={`font-medium ${actionType === "APPROVE" ? "text-black/80" : "text-black/80"}`}>
                                ...
                                <strong className="text-xl inline-block mt-2 text-slate-900">{formatVND(selectedRequest.allocatedAmount)}</strong>
                            </p>
                        </div>
                        ...
                        <div className="p-8 pt-6 space-y-6">
                            {actionType === "REJECT" && (
                                <div className="space-y-2">
                                    ...
                                    <textarea 
                                        className="w-full bg-[#FFFFFF] border border-slate-200 focus:border-rose-400/50 focus:ring-0 rounded-xl p-4 min-h-[100px] text-sm font-medium text-slate-900 placeholder:text-[#94A3B8]"
                            ...
                            <div className="flex gap-4">
                                <button 
                                    className="flex-1 py-4 font-bold rounded-2xl text-slate-900 bg-[#FFFFFF] hover:bg-[#F1F5F9] transition-colors"
