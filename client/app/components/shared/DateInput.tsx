  ...
  return (
    <div className={`relative w-full group ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
      <input
        ...
        disabled={disabled}
        maxLength={10}
        className="w-full px-[0.875rem] py-[0.625rem] bg-[#FFFFFF] border border-slate-200 rounded-[0.75rem] text-slate-900 text-[0.875rem] font-bold outline-none transition-all focus:border-[#2563EB]"
      />
      <div className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-900 pointer-events-none text-[0.6875rem] font-black uppercase tracking-widest opacity-30">
        DD/MM/YYYY
      </div>
    </div>
  );
