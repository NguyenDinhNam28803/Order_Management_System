...
export default function BudgetHeatmap() {
    ...
    const getHeatColor = (ratio: number) => {
        ...
        if (ratio >= 40) return "bg-[#2563EB]/80 border-[#2563EB] shadow-[#2563EB]/20";
    ...
    const getTagColor = (ratio: number) => {
        ...
        if (ratio >= 40) return "bg-[#2563EB]/20 text-[#2563EB] border border-[#2563EB]/30";
        ...
        <div className="bg-[#F1F5F9] border border-[rgba(148,163,184,0.1)] shadow-xl shadow-[#2563EB]/5 overflow-hidden group">
            <div className="p-6 border-b border-[rgba(148,163,184,0.1)] flex items-center justify-between bg-[#1A1D26]">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-lg font-black text-[#F8FAFC] tracking-tight uppercase">Budget Consumption Heatmap</div>
                        <p className="text-[10px] font-black text-[#F8FAFC]/70 uppercase tracking-[0.2em] mt-0.5">Real-time Department Spending Intensity</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                   <div className="flex items-center gap-2">
                        <div className="flex -space-x-1.5">
                            ...
                            <div className="h-4 w-4 bg-[#2563EB] rounded-full border-2 border-[#1A1D26] shadow-sm"></div>
                        ...
                        <span className="text-[10px] font-black text-[#F8FAFC] uppercase tracking-widest ml-1">Intensity Scale</span>
                   </div>
                </div>
            </div>
                    ...
                    >
                        ...
                        <div className="relative z-10 flex flex-col h-full justify-between">
                            <div className="flex justify-between items-start">
                                <div className="h-10 w-10 rounded-[16px] bg-white/10 backdrop-blur-xl flex items-center justify-center text-[#F8FAFC] border border-white/20 shadow-lg">
                                    <ArrowUpRight size={18} />
                                </div>
                                <span className="text-[11px] font-black px-3 py-1 rounded-full backdrop-blur-xl border border-white/20 text-[#F8FAFC] shadow-sm font-mono tracking-tighter">
                                    {item.ratio.toFixed(1)}%
                                </span>
                            </div>
                            
                            <div className="mt-auto">
                                <div className="text-[10px] font-black text-[#F8FAFC]/60 uppercase tracking-[0.2em] mb-1.5 leading-none">{item.code}</div>
                                <div className="text-sm font-black text-[#F8FAFC] leading-tight break-words drop-shadow-sm" title={item.name}>{item.name}</div>
