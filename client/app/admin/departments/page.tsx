...
export default function DepartmentsPage() {
    const { departments, users, addDept, updateDept, organizations } = useProcurement();
    ...
    const handleOpenModal = (dept?: any) => {
        if (dept) {
            ...
            setFormData({
                ...
                budgetAnnual: Number(dept.budgetAnnual) || 0,
            });
        } else {
            setEditingDept(null);
            setFormData({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const data = {
            ...formData,
            budgetAnnual: Number(formData.budgetAnnual),
        };
                        ...
                        <tbody>
                            {departments?.map((dept: any) => (
                                <tr key={dept.id} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50">
                                    ...
                                    <td>
                                        <div className="font-black text-erp-navy">
                                            {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(dept.budgetAnnual || 0)}
                                        </div>
                                        <div className="text-[9px] text-slate-400 font-bold">NGÂN SÁCH CÒN LẠI: 85%</div>
                                    </td>
                    ...
                    <div className="bg-white rounded-[40px] w-full max-w-xl overflow-hidden shadow-2xl border border-white/20">
                        <div className="p-10">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Ngân sách năm (VND)</label>
                                    <input 
