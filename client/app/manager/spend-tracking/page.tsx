        ...
        <main className="animate-in fade-in duration-500 p-6 min-h-screen bg-[#0F1117] text-[#F8FAFC]">
                ...
                <div className="flex gap-4 mt-6 lg:mt-0">
                    <div className="relative group">
                        <select 
                            ...
                            className="bg-[#161922] border border-[rgba(148,163,184,0.1)] hover:border-[#3B82F6]/50 rounded-xl text-[10px] font-black uppercase tracking-widest text-[#F8FAFC] px-4 py-3 pr-10 outline-none appearance-none cursor-pointer transition-all min-w-[200px] focus:border-[#3B82F6] focus:ring-1 focus:ring-[#3B82F6]/20"
                        >
                            <option value="ALL" className="bg-[#161922] text-[#F8FAFC]">Tất cả Trung tâm chi phí</option>
                            {myCostCenters.map(cc => (
                                <option key={cc.id} value={cc.id} className="bg-[#161922] text-[#F8FAFC]">
                                    {cc.name} ({cc.code})
                                </option>
                            ))}
                        </select>
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[#64748B] group-hover:text-[#3B82F6] transition-colors">
                            <ChevronDown size={14} />
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-[#161922] rounded-3xl border border-[rgba(148,163,184,0.1)] shadow-xl shadow-[#3B82F6]/5 overflow-hidden">
                    <div className="px-6 py-5 border-b border-[rgba(148,163,184,0.1)] bg-[#0F1117]">
                        <h4 className="text-xs font-black uppercase tracking-widest text-[#F8FAFC] leading-none">Phân Bổ Ngân Sách</h4>
                    </div>
                    <div className="p-6">
                        <DonutChart 
                            title=""
                ...
                <div className="bg-[#161922] rounded-3xl border border-[rgba(148,163,184,0.1)] shadow-xl shadow-[#3B82F6]/5 overflow-hidden">
                    <div className="px-6 py-5 border-b border-[rgba(148,163,184,0.1)] bg-[#0F1117] flex items-center justify-between">
                        <h4 className="text-xs font-black uppercase tracking-widest text-[#F8FAFC] leading-none">Chi Tiêu Theo Cost Center</h4>
                        {filteredAllocations.length > 5 && (
                            <span className="text-[10px] font-bold text-[#64748B]">Top 5 / {filteredAllocations.length}</span>
                        )}
                    </div>
                    <div className="p-6">
                        <SimpleBarChart 
                    ...
                    {filteredAllocations.length > 5 && (
                        <div className="px-6 py-3 bg-[#0F1117] border-t border-[rgba(148,163,184,0.1)]">
                            <p className="text-[10px] text-[#64748B] text-center">
                                Hiển thị top 5 cost center chi tiêu nhiều nhất • 
                                <span className="text-[#3B82F6] font-bold">{filteredAllocations.length - 5} cost center khác</span>
                            </p>
                        </div>
                    )}
                </div>
                    ...
                    <table className="erp-table text-xs whitespace-nowrap">
                        <thead>
                            <tr className="bg-[#0F1117]">
                                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-[#64748B]">Cost Center</th>
                                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-[#64748B]">Phân bổ</th>
                                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-[#64748B] text-right">Cam kết</th>
                                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-[#64748B] text-right">Thực chi</th>
                                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-[#64748B] text-center">Tiến độ</th>
                                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-[#64748B] text-right">Còn lại</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[rgba(148,163,184,0.1)]">
                            {paginatedAllocations.length > 0 ? paginatedAllocations.map((alloc) => {
                                ...
                                return (
                                    <tr key={alloc.id} className="hover:bg-[#0F1117]/50 transition-colors group">
                                        <td className="py-5 px-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-[11px] font-black text-[#F8FAFC] leading-tight">{cc?.name || "N/A"}</span>
                                                    <span className="text-[9px] font-bold text-[#64748B] uppercase tracking-tight">{cc?.code || "N/A"}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-5 px-6 font-bold text-[#F8FAFC]">{formatVND(Number(alloc.allocatedAmount))}</td>
                                        <td className="py-5 px-6 text-right text-[#3B82F6] font-bold">{formatVND(Number(alloc.committedAmount))}</td>
                                        <td className="py-5 px-6 text-right font-bold text-[#F8FAFC]">{formatVND(Number(alloc.spentAmount))}</td>
                                ...
                                <tr><td colSpan={6} className="py-12 text-center text-[#64748B] font-black uppercase text-[10px]">Không có dữ liệu phân bổ</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between px-6 py-4 bg-[#0F1117] border-t border-[rgba(148,163,184,0.1)]">
                        <div className="text-[10px] text-[#64748B]">
                            Hiển thị <span className="font-bold text-[#F8FAFC]">{paginatedAllocations.length}</span> / <span className="font-bold text-[#F8FAFC]">{filteredAllocations.length}</span> phân bổ
                        </div>
                        <div className="flex items-center gap-2">
                            <button
